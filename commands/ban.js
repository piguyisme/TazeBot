exports.run = (client, message, args) => {
  ...
  if (subject !== undefined) {
    ...
    if (message.member.roles.has(staff[0]) && !subject.roles.has(staff[0]) && !subject.roles.has(staff[1]) && !subject.roles.has(staff[2]) && !subject.roles.has(staff[3]) && !subject.roles.has(staff[4])) { accepted = true; }
      ...
      message.channel.send("You dont have permission. You can apply for staff [here](https://goo.gl/forms/J7G1JsCDoJfsDxOz1)").catch(console.error);
    }
  }
  else { message.channel.send("You didn't specify someone to ban.").catch(console.error); }
  if (accepted) {
    subject.ban();
    message.channel.send("The Ban Hammer has Struck!").catch(console.error);
    const embed = new Discord.RichEmbed()
      .setAuthor(message.author.username, message.author.avatarURL)
      .setTitle("User Banned")
      .setDescription(`${subject.displayName} banned by ${message.author}`)
      .setTimestamp()
